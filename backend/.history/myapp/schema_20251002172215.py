...
class UpdateDepartment(graphene.Mutation):
    ...
    class Arguments:  # ✅ fixed
        id = graphene.String(required=True)
        image = graphene.String(required=False)
        lead = graphene.String(required=True)
        name = graphene.String(required=True)
    
    def mutate(self, info, id, name, image=None, lead=None):  # ✅ fixed
        print("Name:", name)
        print("Lead:", lead)
        print("Image preview:", image[:50]) 
        user = info.context.user
        ...
        try:
            department = Departments.objects.get(id=id, user=user)
...
class Query(graphene.ObjectType):
    ...
    def resolve_departments(self, info):
        ...
        if user.is_anonymous:
            raise Exception("Authentication failure: You must be signed in.")
        return Departments.objects.filter(user=user) 
