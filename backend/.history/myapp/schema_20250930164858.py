...
class DepartmentType(DjangoObjectType):
    class Meta:
        model = User
        fields = ("id", "name", "user", "image","lead")
...
class CreateDepartment(graphene.Mutation):
    ...
    class Arguments:
        ...
        image = graphene.String(required=True)  # Base64 string or image URL
        lead = graphene.ID(required=True)       # Assuming you pass lead ID

    def mutate(self, info, name, image, lead):
        # Get the user by ID (lead)
        try:
            lead_user = User.objects.get(pk=lead)
        except User.DoesNotExist:
            raise Exception("Invalid lead user ID")
