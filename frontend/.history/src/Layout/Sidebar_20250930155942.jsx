...
export default function Sidebar({ children }) {
  ...
  const navItems = [
    { name: 'Home', icon: 'bi-house-door-fill', redirect: '/', className:'dashboard' },
    { name: 'Departments', icon: 'bi-house-door-fill', redirect: '/Department', className:'department' },
  ];
    ...
    >
      ...
      <ul className="nav nav-pills flex-column mb-auto">
        {navItems.map((item, index) => ( 
          <NavLink to={item.redirect} key={item.name || index}>
           <li className="nav-item mb-2" key={index}>
            <div
              className={`nav-link d-flex align-items-center ${item.className} ${index === activeIndex ? 'active' : 'text-secondary'}`}
              style={{
