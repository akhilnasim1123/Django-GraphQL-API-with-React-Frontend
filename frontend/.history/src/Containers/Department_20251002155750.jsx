...
function DepartmentsPage() {
  ...
  const handleCreateDepartment = async (e) => {
    ...
    try {
        ...
        const department = await dispatch(createDepartment({ name, image: base64Image, lead, user })).unwrap();
                                ...
                                {departments.map((dept, index) => (
                                    <tr key={dept.id}>
                                        ...
                                        <td className="align-middle border-secondary">
                                            <img
                                                src={dept.image}
                                                alt={`http://192.168.18.40:8000/${dept.image} logo`}
                                                className="rounded-circle"
                                                style={{ width: '40px', height: '40px' }}
                                            />
                                        </td>
                                        <td className="align-middle border-secondary text-">{dept.lead}</td>
