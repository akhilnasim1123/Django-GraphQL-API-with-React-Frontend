  ...
  async ({ username, password }, thunkAPI) => {
    try {
      ...
      if (data?.tokenAuth?.token) {
        ...
        // await thunkAPI.dispatch(getUser(data.payload.username));
...
export const getDepartments = createAsyncThunk(
  ...
  async (user, thunkAPI) => {
    try {
      ...
      if (data.departments) {
        return data.departments;  // ✅ Return data so it reaches `.unwrap()` or your reducer
      } else {
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getDepartments.fulfilled, (state, action) => {
  state.loading = false;
  state.departments = action.payload; // ✅ Save fetched data
})
  },
});
