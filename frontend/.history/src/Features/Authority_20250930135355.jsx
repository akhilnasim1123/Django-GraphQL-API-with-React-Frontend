  ...
  async ({ username, password }, thunkAPI) => {
    try {
      ...
      if (data?.tokenAuth?.token) {
        ...
        console.log(data)
        console.log(data.tokenAuth.refreshToken) 
        const whoamiRes = await client.query({
          query: WHOAMI,
          context: {
            headers: {
              Authorization: `JWT ${data.tokenAuth.refreshToken}`,
            },
          },
          fetchPolicy: 'no-cache',
        });
        console.log(whoamiRes) 
        const user = whoamiRes?.data?.whoami;
        console.log(user)
        return {
          token,
          refreshToken,
